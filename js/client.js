...
let completedDays = JSON.parse(localStorage.getItem('fitnessCompletedDays')) || new Array(TOTAL_DAYS).fill(false);
let cargas = JSON.parse(localStorage.getItem('fitnessCargas')) || {};
...
function guardarEvaluacion(semana) {
...
evaluacionNombre = "Final";
} else if (evalSemana2) {
const data = JSON.parse(evalSemana2);
pesoActual = data.peso;
musculoActual = data.musculo;
grasaActual = data.grasa;
evaluacionNombre = "Semana 2";
} else if (evalSemana1) {
...
function guardarCheckin(estado) {
